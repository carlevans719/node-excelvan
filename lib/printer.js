...
class Printer extends events.EventEmitter {
  ...
  disconnect (callback = () => {}) {
    this._device.interfaces[ 0 ].release(error => {
      ...
      this._device.interfaces[ 0 ].release(true, () => {
        try {
  ...
  print (printJob, callback = () => {}) {
    ...
    const inEndpoint = this._device.interfaces[ 0 ].endpoints[ 0 ]
    const outEndpoint = this._device.interfaces[ 0 ].endpoints[ 1 ]
    const packetSize = outEndpoint.descriptor.wMaxPacketSize || 64
    let printData = printJob.printData ? printJob.printData() : printJob // handle raw data
    ...
    printData.forEach(buf => {
      const packets = []
      const packetCount = Math.ceil(buf.length / packetSize)

      for (let i = 0; i < packetCount; i++) {
        let packet = new Buffer(packetSize)

        packet.fill(' ')
        buf.copy(packet, 0, i * packetSize, (i + 1) * packetSize)
        packets.push(packet)
      }
